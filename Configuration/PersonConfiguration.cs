...
public class PersonConfiguration : IEntityConfiguration
{
    public bool Configure(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<Person>().Property(p => p.FirstName).IsRequired().HasColumnType("Nvarchar(50)").HasAnnotation("MinLength", 2);
        modelBuilder.Entity<Person>().Property(p => p.LastName).IsRequired().HasColumnType("Nvarchar(50)").HasAnnotation("MinLength", 2);
        modelBuilder.Entity<Person>().Property(p => p.PersonalNumber).IsRequired().HasColumnType("Nvarchar(11)").HasAnnotation("MinLength", 11);
        ...
        /* Picture */
        modelBuilder.Entity<Person>().HasMany(p => p.PhoneNumbers).WithOne(c => c.Person);
